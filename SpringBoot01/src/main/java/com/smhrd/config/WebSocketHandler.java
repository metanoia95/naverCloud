...
// 이 클래스가 Bean으로 등록이 되어져야 한다. 
@Component // Bean에 특별한 기능 없이 등록만 되는 것.
public class WebSocketHandler extends TextWebSocketHandler {
	...
	// >> key, value 자료형 처음 한번만 정의, 변하지않음.

	private static HashMap<String, List<WebSocketSession>> users = new HashMap<String, List<WebSocketSession>>();
	...
	public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
		...
		// (socket이 메세지를 받았을 때)
		// session : 메세지를 보낸 사용자의 정보 / message : 사용자정보
		message.getPayload();
